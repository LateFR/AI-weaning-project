...
class SimpleScraper():
    ...
    def get_html(self, url):
        if self.url_validity_check(url):
            try:
                ...
                logger.error(f"Fatal: the web page can be reached. Details: {e}")
                exit()
        else:
    ...
    def scrap(self, html):
        try:
            ...
            logger.debug(f'Launching find_all with the tag "{self.tag}", the pattern "{regex}" and src on "{self.with_src}"')
            matches = soup.find_all(self.tag, string=regex, src=self.with_src, recursive=True)
            ...
            logger.error(f"The web page's code can't be extracted. Details: {e}")
    ...
    def run(self):
        ...
        self.writes_results(html, matches)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--only-src", "-os", action="store_true", help="To get only tag with an src link")
